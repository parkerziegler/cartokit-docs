...
The **Properties Panel** contains a handful of controls for styling the layer. The most powerful of these is the **Layer Type** select, which controls the core cartographic representation of the layer. By default, `cartokit` uses the **Polygon** type to style `Polygon` and `MultiPolygon` geometries. We'll switch to a **Choropleth** type, which will allow us to color each county according to its unemployment rate.

<Video src="/transition-layer-type.mp4" />
